...
<script>
  ...
  require('echarts/extension/bmap/bmap');
    export default {
      ...
      mounted(){
        console.log("want_jobfunction",this.want_jobfunction);
        if (this.want_jobfunction === 2){
          this.want_jobfunction = '';
        }
        this.getJobMap();
      },
      methods:{
        getJobMap(){
          getJobsMap({
            ordering: '-job_count',
            jobfunction: this.want_jobfunction
          }).then((response)=> {
        ...
        drawMyMap(){
          ...
          for( let city of this.cities){
            ...
            if (geoCoord) {
              results.push({
                name: city['city_name'],
                value: geoCoord.concat(city['job_count'],(city['job_salary_low']+city['job_salary_high'])/2)
              });
            }
          }
          let option = {
